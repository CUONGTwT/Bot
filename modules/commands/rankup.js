...
module.exports.event = async function ({ api, event, Currencies, Users, client }) {
	...
	if (level > curLevel && level != 1) {
		...
		var messsage = (typeof threadData.customRankup == "undefined") ? msg = "Chúc mừng {name} đã lên rank rank hiện tại của {name} là cao thủ {level} ⭐️" : msg = threadData.customRankup,
			arrayContent;
		...
		if (existsSync(__dirname + `/cache/rankup/1.mp3`)) arrayContent = { body: messsage, attachment: createReadStream(__dirname + `/cache/rankup/1.mp3`), mentions: [{ tag: nameUser, id: senderID }] };
